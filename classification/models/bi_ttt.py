...
def scan(f, init, xs, out, checkpoint_group=0):
    ...
    if checkpoint_group > 0 and (num_items // checkpoint_group) > 0:
...
class TTTBase(nn.Module):
    ...
    def forward(
        ...
        # feature, classtoken = hidden_states[:, :-1], hidden_states[:, -1:]
        # feature_reverse = torch.flip(feature, dims=[1])
        # hidden_states_reverse = torch.cat([feature_reverse, classtoken], dim=1)
        # hidden_states = torch.cat([hidden_states, hidden_states_reverse], dim=0)
        # B *= 2
        ...
        if position_ids is None:
            position_ids = torch.arange(L, device=XV.device).unsqueeze(0)
        ...
        # reverse_hidden_states = lambda h: torch.cat([
        #     h,
        #     torch.cat([torch.flip(h[:, :-1], dims=[1]), h[:, -1:]], dim=1)
        # ], dim=0)

        # X_list = [XQ, XK, XV, hidden_states]
        # XQ, XK, XV, hidden_states = [reverse_hidden_states(h) for h in X_list]
        # B *= 2
        ...
        # output_hidden_states, output_hidden_states_reverse = torch.chunk(output_hidden_states, 2, dim=0)
        # output_feature_reverse, output_classtoken_reverse = output_hidden_states_reverse[:, :-1], output_hidden_states_reverse[:, -1:]
        # output_hidden_states_reverse = torch.cat([torch.flip(output_feature_reverse, dims=[1]), output_classtoken_reverse], dim=1)

        # output_hidden_states = output_hidden_states + output_hidden_states_reverse

        # hidden_states, hidden_states_reverse = torch.chunk(hidden_states, 2, dim=0)
        # feature_reverse, classtoken_reverse = hidden_states_reverse[:, :-1], hidden_states_reverse[:, -1:]
        # hidden_states_reverse = torch.cat([torch.flip(feature_reverse, dims=[1]), classtoken_reverse], dim=1)
        # hidden_states = (hidden_states + hidden_states_reverse)/2
        ...
        if self.use_gate:
            ...
            # output_hidden_states = self.apply_gate(hidden_states_residual, output_hidden_states)
            # output_hidden_states_reverse = self.apply_gate(hidden_states_residual, output_hidden_states_reverse)

        # output_hidden_states = (output_hidden_states + output_hidden_states_reverse)/2
...
class TTTLinear(TTTBase):
    ...
    def ttt(
        ...
        # last_mini_batch_params_dict 上一个内部循环参数字典？
        ...
        def compute_mini_batch(params_dict, inputs):
            ...
            # why num head / minibatch

            # [B,nh,K,f], K=mini_batch_size
            XQ_mini_batch = inputs["XQ"]
            XV_mini_batch = inputs["XV"]
            XK_mini_batch = inputs["XK"]
            # multi view parameter
            ...
            # compute recostruction loss
            # print(reconstruction_target)
            # exit()
        ...
        # 内部参数, 输出
        New_W1 = init_params_dict["W1_states"].data.mean(dim=0)
        New_b1 = init_params_dict["b1_states"].data.mean(dim=0)

        belta = 0.999999
        self.W1.data = belta * self.W1.data + (1 - belta) * New_W1
        self.b1.data = belta * self.b1.data + (1 - belta) * New_b1
...
# class BiTTTLinear(nn.Module):
#     def __init__(self, config=None, layer_idx=0):
#         super().__init__()
#         self.attn_rf = TTTLinear(config=config, layer_idx=layer_idx)
#         self.attn_rb = TTTLinear(config=config, layer_idx=layer_idx)
#         self.out_proj = nn.Linear(config.hidden_size*2, config.hidden_size, bias=True)
    
#     def forward(self, x):
#         rf_x = self.attn_rf(x)
#         rb_input = torch.cat([torch.flip(x[:, :-1], dims=[1]), x[:, -1:]], dim=1)
#         rb_x = self.attn_rb(rb_input)
#         rb_x = torch.cat([torch.flip(rb_x[:, :-1], dims=[1]), rb_x[:, -1:]], dim=1)
#         x = self.out_proj(torch.cat([rf_x, rb_x], dim=-1))
#         return x
...
#         super().__init__()
#         d_conv = 3
#         self.pre_conv = nn.Conv2d(in_channels=config.hidden_size, 
...
#                                   padding=(d_conv - 1) // 2,)
#         self.act = nn.SiLU()
#         self.norm_token=nn.LayerNorm(config.hidden_size)
#         self.norm_cls=nn.LayerNorm(config.hidden_size)
#         self.norm_bi=nn.LayerNorm(config.hidden_size*2)
#         self.attn_rf = TTTLinear(config=config, layer_idx=layer_idx)
#         self.attn_rb = TTTLinear(config=config, layer_idx=layer_idx)
#         self.out_proj = nn.Linear(config.hidden_size*2, config.hidden_size, bias=True)
...
#         H_W = L - 1
#         H = W = int(H_W**0.5)
#         class_token, x_without_cls = x[:, -1:, :], x[:, :-1, :]
#         x_reshaped = x_without_cls.transpose(1, 2).reshape(B, C, H, W)
#         x_processed = self.pre_conv(x_reshaped)  # [B, out_channels, H, W]
#         x_processed = self.act(x_processed).view(B, -1, H * W).transpose(1, 2)
#         x = torch.cat([self.norm_token(x_processed), self.norm_cls(class_token)], dim=1)

#         rf_x = self.attn_rf(x)
#         rb_input = torch.cat([torch.flip(x[:, :-1], dims=[1]), x[:, -1:]], dim=1)
#         rb_x = self.attn_rb(rb_input)
#         rb_x = torch.cat([torch.flip(rb_x[:, :-1], dims=[1]), rb_x[:, -1:]], dim=1)
#         x = self.out_proj(self.norm_bi(torch.cat([rf_x, rb_x], dim=-1)))
#         return x
...
class BiTTTLinear(nn.Module):
    def __init__(self, config=None, layer_idx=0):
        ...
        self.pass_conv = nn.Conv2d(in_channels=config.hidden_size, 
                                  out_channels=config.hidden_size, 
                                  groups=config.hidden_size,
                                  bias=True,
                                  kernel_size=3,
    ...
    def forward(self, x):
        ...
        x_conv = self.pass_conv(x_reshaped) 
        x_conv = x_conv.view(B, -1, H * W).transpose(1, 2)
        x_conv = torch.cat([x_conv, class_token], dim=1)
        ...
        rb_x = self.attn_rb(rb_input)
        rb_x = torch.cat([torch.flip(rb_x[:, :-1], dims=[1]), rb_x[:, -1:]], dim=1)
        x = self.out_proj(self.norm_bi(torch.cat([rf_x, rb_x], dim=-1))) + x_conv
        return x

# class MRF(nn.Module):
#     def __init__(self, hidden_size, d_conv=3):
#         super().__init__()
#         # 空洞卷积的不同空洞率
...
#             dilation=1
#         )
#         self.conv_r3 = nn.Conv2d(
#             in_channels=hidden_size,
#             out_channels=hidden_size,
#             groups=hidden_size,
#             bias=True,
#             kernel_size=3,
#             padding=(d_conv - 1) * 3 // 2,
#             dilation=3
#         )
#         self.conv_r5 = nn.Conv2d(
#             in_channels=hidden_size,
#             out_channels=hidden_size,
#             groups=hidden_size,
#             bias=True,
#             kernel_size=3,
#             padding=(d_conv - 1) * 5 // 2,
#             dilation=5
#         )
#         # 激活函数
#         self.act = nn.SiLU()
#         # 降维的线性层
#         self.conv_out_proj = nn.Linear(hidden_size * 3, hidden_size, bias=True)
...
#         x_r5 = self.conv_r5(x)
#         # 拼接
#         x_combined = torch.cat([x_r1, x_r3, x_r5], dim=1)  # [B, C*3, H, W]
#         # 调整形状
#         x_combined = x_combined.view(B, -1, H * W).transpose(1, 2)  # [B, H*W, C*3]
        
#         class_token = class_token.permute(0, 2, 1).reshape(B, C, 1, 1)
#         class_token_r1 = self.conv_r1(class_token)
#         class_token_r3 = self.conv_r3(class_token)
#         class_token_r5 = self.conv_r5(class_token)

#         # 拼接
#         class_token_combined = torch.cat([class_token_r1, class_token_r3, class_token_r5], dim=1)  # [B, C*3, 1, 1]
#         # 调整形状
#         class_token_combined = class_token_combined.view(B, C*3, 1).transpose(1, 2)  # [B, 1, C*3]

#         # 拼接 class_token
#         x_combined = torch.cat([x_combined, class_token_combined], dim=1)  # [B, H*W+1, 3*C]
#         x_combined = self.act(x_combined)
#         # 降维
#         x_combined = self.conv_out_proj(x_combined)  # [B, H*W, C]
#         return x_combined
...
#         d_conv = 3
#         # 添加 DilatedConvBlock
#         self.mrf = MRF(config.hidden_size, d_conv)
...
#         # 使用 DilatedConvBlock
#         x_conv = self.mrf(x_reshaped, class_token)
...
#         # 最终加和
#         x = self.out_proj(self.norm_bi(torch.cat([rf_x, rb_x], dim=-1))) + x_conv * self.alpha
#         return x
