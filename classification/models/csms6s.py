...
def selective_scan_torch(
    u: torch.Tensor, # (B, K * C, L)            # torch.Size([1, 384, 3136])
    delta: torch.Tensor, # (B, K * C, L)        # torch.Size([1, 384, 3136])
    A: torch.Tensor, # (K * C, N)               # torch.Size([384, 1])
    B: torch.Tensor, # (B, K, N, L)             # torch.Size([1, 4, 1, 3136])
    C: torch.Tensor, # (B, K, N, L)             # torch.Size([1, 4, 1, 3136])
    D: torch.Tensor = None, # (K * C)           # torch.Size([384])
    delta_bias: torch.Tensor = None, # (K * C)  # torch.Size([384])
    delta_softplus=True, 
    oflex=True, 
    *args,
    **kwargs
):
    dtype_in = u.dtype    # torch.float32
    Batch, K, N, L = B.shape
    KCdim = u.shape[1]    # Kdim=384
    Cdim = int(KCdim / K) # Cdim=96
    assert u.shape == (Batch, KCdim, L)
    assert delta.shape == (Batch, KCdim, L)
    assert A.shape == (KCdim, N)
    assert C.shape == B.shape

    # ...（省略号）表示 保持所有已有维度不变，只在最后新增一个维度。
    # None 等价于 unsqueeze(-1)，会在 最后一个维度添加一个新轴。
    # delta_bias[..., None]扩展一个维度 torch.Size([384，1])
    # 再通过广播机制 torch.Size([1，384，3136])
    m=delta_bias[..., None]
    ...
    # deltaA torch.Size([1, 384, 3136, 1])
    deltaA = torch.exp(torch.einsum('bdl,dn->bdln', delta, A))
    # deltaB_u torch.Size([1, 384, 3136, 1])
    ...
    if True:
        ...
        for i in range(L):                                          # 循环L次
            x = deltaA[:, :, i, :] * x + deltaB_u[:, :, i, :]       # 更新 x
            y = torch.einsum('bdn,bdn->bd', x, C[:, :, :, i])       # 计算 x 和 C 的点积
            ys.append(y)
        # torch.Size([1, 384, 3136])
