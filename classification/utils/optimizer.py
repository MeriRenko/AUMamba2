...
def build_optimizer(config, model, logger, simmim=False, is_pretrain=False, **kwargs):
    ...
    if simmim:
        ...
        if kwargs['args'].pretrained:
            parameters, no_decay_names = set_weight_decay_lr(model, skip, skip_keywords, lr=config.TRAIN.BASE_LR)
        else:
...
def set_weight_decay_lr(model, skip_list=(), skip_keywords=(), lr=1e-3):
    has_decay = []
    no_decay = []
    head_has_decay = []
    head_no_decay = []
    ...
    for name, param in model.named_parameters():
        if not param.requires_grad:
            continue  # 冻结的权重，不参与更新
        if "head" in name or "classifier" in name or "upsampler" in name:
            # 这是模型的头部参数
            if len(param.shape) == 1 or name.endswith(".bias") or (name in skip_list) or \
                    check_keywords_in_name(name, skip_keywords):
            ...
            if len(param.shape) == 1 or name.endswith(".bias") or (name in skip_list) or \
                ...
                has_decay.append(param)
    return [
        {'params': has_decay, 'lr': lr * 0.1},
        {'params': no_decay, 'weight_decay': 0., 'lr': lr * 0.1},
        {'params': head_has_decay, 'lr': lr},
        {'params': head_no_decay, 'weight_decay': 0., 'lr': lr}
    ], no_decay_names
